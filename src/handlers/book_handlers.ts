...
const index = async (_req: Request, res: Response)=>{   //add _ before req, because we are not going to use it here.
    try {
...
const show = async (req: Request, res: Response)=>{
    try {
        const result = await book.show(req.body.id)
        res.json(result);
    } catch (error) {
...
const destroy = async (req :Request , res :Response)=>{
    try {
        const result = await book.delete(req.body.id);
        res.json(result);
    } catch (error) {
...
const create = async (req: Request, res: Response) => {
    try {
        const result = await book.create(req.body.id);
        res.json(result);
    } catch (error) {
...
export const index_route = (app : express.Application)=>{
    app.get('/book', index);
}
